...
you haven't). FIRST, read the mode: `bash .skill/flow/runner/flow.sh mode`
...
`bash .skill/flow/runner/flow.sh auto` — proceed only on PREFLIGHT OK.
...
1. ALWAYS run the deterministic checker first:
   ```
   bash .skill/flow/runner/flow.sh <subcommand>
   ```
